...
// ---------------AGREGA LA FUNCION DE ARRANQUE DEL JUEGO--------------
function startGame()
{
...
function game(){
  ...
  //-----------MUESTRA EN LA CONSOLA LA POCISION ACTUAL DE LA NARIZ----------------
  console.log(" ");
  ...
  //---------QUE SUCEDE CUANDO EL JUEGO YA HA INICIADO--------------------------------
  ...
  if(gameConfig.status==='start'){
    ...
    // -----------CAMBIA EL MENSAJE PARA QUE EL USUARIO SEPA QUE SE JUEGA CON LA NARIZ-------------
...
//----------CARGA LA MUSICA DE WORLD START PARA QUE SUENE AL INICIAR EL JUEGO -------------
...
function getCoins(coin,character){
  if( character.overlap(coin) && character.live && coin.get==false){
    ...
    //--------------AGREGA EL SONIDO CUANDO MARIO TOMA UNA MONEDA ------------------
...
function manualControl(character){
  ...
  if(character.live){
    ...
    // ------ CAMBIA LA CONDICION DE CONTROL DEL JUEGO USANDO LA POSICION DE LA NARIZ---------------
    if(CONDICION){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
...
function jumping(character){
   ...
   //----------------------------------------------------------------------- 
    ...
    // CAMBIA POR LOS CONTROLES CON NARIZ EN X y EN Y
	if( (CONDICION  &&character.live) || (touchIsDown&&character.live) ){
    character.velocity.y+=gameConfig.jump;



    //------------------AGREGA EL SONIDO CUANDO MARIO SALTA ------------------
...
function StepOnEnemy(obj1,obj2){
    ...
    //--------------AGREGA SONIDO PARA CUANDO MARIO MATA UN ENEMIGO------------------
...
function die(character){
    ...
    if(character.liveNumber > 0)
    {
      ...
      //------------AGREGA SONIDO PARA CUANDO MARIO MUERE ------------------
...
function checkStatus(character){    
  ...
  if(character.live==false && character.liveNumber==0){
    ...
    //------------AGREGA SONIDO PARA CUANDO MARIO PIERDE EL JUEGO-----------------
